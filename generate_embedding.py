...
if not os.path.exists("/data/save_index"):
    os.mkdir("/data/save_index")
...
class GenerateEmbeddingHandler(tornado.web.RequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            if action=="delete":
                if os.path.exists("/data/save_index/%s_index" % filepath.replace("/","_")):
                    ret["response"] = "delete index error"
                else:
                    ret["response"] = "delete index done"
            else:
                if os.path.exists("/data/save_index/%s_index" % filepath.replace("/","_")):
                    ret["response"] = "update/add index done"
                else:
                    ret["response"] = "update/add index error"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--back_url", type=str,default="")
